...
public:
	...
	friend void control(int i,int k,int x,int s,television &a);
};
int television::TV = 0;
int television::DVD = 0;
void control(int i, int k, int x, int s, television& a)
{
	a.mode = k;
	if (k == 1)
	{
	...
	if (k == 1)
	{
		cout << "第" << i << "号电视机--TV模式--频道" << a.channel << "--音量" << a.sound << endl;
	}
	else if (k == 2)
	{
		cout << "第" << i << "号电视机--DVD模式--频道" << a.channel << "--音量" << a.sound << endl;
	}
}
int main()
{
	...
	for (int i = 0; i < n; i++)
	{
		if (p[i].getmode() == 1)
			television::TV++;
		else if(p[i].getmode() == 2)
			television::DVD++;
	}
	cout << "播放电视的电视机数量为" << television::TV << endl;
	cout << "播放DVD的电视机数量为" << television::DVD << endl;
	return 0;
}/*题目描述
